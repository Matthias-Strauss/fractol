...
/*   Updated: 2024/04/26 19:17:31 by mstrauss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	redraw(t_fractol *fractol)
{
	...
	else if (fractol->fractal_set == 2 || fractol->fractal_set == 3)
		julia(fractol->math, fractol);
	else if (fractol->fractal_set == 4)
		burning_ship(fractol->math, fractol);
}
...
int	get_param(t_math *math, int argc, char **argv, t_fractol *fractol)
{
	...
	else if ((!ft_strncmp(argv[1], "-j", 2) || !ft_strncmp(argv[1], "--julia",
				7)) && argc == 2)
	{
		math->ca = 0.355;
		math->cb = 0.355;
	...
	else if ((ft_strncmp(argv[1], "-j", 2) || ft_strncmp(argv[1], "--julia", 7))
		&& argc == 4)
	{
	...
	else if (ft_strncmp(argv[1], "-bs", 3) || ft_strncmp(argv[1],
			"--burning-ship", 14))
	{
		math->ca = 0;
		math->cb = 0;
		return (fractol->fractal_set = 4, 4);
	}
	return (0);
}
